...
Instead of registering by hand, set `enable_suseconnect` to `true` in
your inventory to have every host in the play register the modules above itself.
...
| enable_suseconnect | Whether to automatically register the SCC
  ...
  Set it in your inventory, e.g. under `[all:vars]` or in a `group_vars`/`host_vars` file. | false
