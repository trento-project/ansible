...
| provision_prometheus | Whether to provision Prometheus used by Trento to store
  metrics send by agents. 
  
  Note that the provisioned Prometheus is only intended for use by Trento and not for general monitoring purposes. | true

| enable_prometheus_remote_write_receiver | Whether to start Prometheus with remote write receiver enabled.
