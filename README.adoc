...
| provision_proxy | Provision nginx to expose the services, set to
  false to use an existing reverse proxy deployment | "true"
...
| app_web_postgres_host | Host of the Postgres containing the DB for
...
| app_wanda_postgres_host | Host of the Postgres containing the DB for
  Wanda | localhost
...
| agent_rabbitmq_host | The rabbitmq host, used by web and wanda
  containers. It could include the service port | "localhost"
