...
The following variables control whether this playbook would take care
of provisioning third-party application dependencies by installing
them and/or configuring them. In addition to that, if their value is
`true` they would enable a feature of the playbook that allows for
automatic population of some variables.
...
| provision_postgres | Whether to provision Postgres. If set to
  ...
  `app_wanda_postgres_host` would be ignored and host discovery would
  use the hosts specified in the inventory. Set to `false` if you
...
| provision_rabbitmq | Whether to provision RabbitMQ. If set to
  ...
  would be ignored and host discovery would use the hosts specified in
  the inventory. Set to `false` if you provide an external RabbitMQ to
  the services. | "true"
...
| agent_trento_repository | OBS repository from where Trento agent is
  installed |
...
| prometheus_port | Port where Prometheus would be exposed | 9090

| prometheus_web_url | Base URL of Trento Web application where
  Prometheus would scrape for metrics | http://<`trento_server_name`>
