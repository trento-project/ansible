...
The `postgres`, `prometheus`, `rabbitmq` and `rproxy` roles depend on
the internal `port_check` utility role, which verifies their target
port is free (or already legitimately owned by the service, on a
playbook re-run) before installation proceeds, failing early with a
clear message instead of a confusing package/service error. It is not
meant to be used directly; see `roles/port_check/README.adoc` for
details.
