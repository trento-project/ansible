...
the Trento MCP Server as a systemd service on the `trento_server`
hosts. By default it uses local `OAS_PATH` values to connect to Web
and Wanda on the same host. If `provision_proxy` is also `true`, the
endpoint is exposed through NGINX at
...
| provision_mcp_server | Whether to provision the Trento MCP Server as a
  systemd service. If `provision_proxy` is also `true`, the endpoint
  is exposed through the existing reverse proxy. | false
...
| mcp_server_target_mode | How the Trento MCP Server discovers the
  Trento APIs. Use `oas_path` for same-host systemd deployments, or
  `trento_url` to autodiscover from a Trento base URL. | "oas_path"
