...
Trento-specific data. This is controlled by the
`provision_supportutils` variable.
...
| provision_supportutils | Whether to install `supportutils` and the
  `supportutils-plugin-trento` package on Trento server hosts. Set to
  `false` if you do not want the Trento support plugin installed. | true

|===

To disable the Trento support plugin during provisioning, set:

[source,yaml]
----
provision_supportutils: false
----
...
the `cleanup.yml` playbook could be used. It uses the same
