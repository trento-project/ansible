...
The `server.yml` playbook invokes the internal `port_check` utility
role before provisioning the `postgres`, `prometheus`, `rabbitmq` and
`rproxy` roles. It verifies that each target port is free (or already
legitimately owned by the service, on a playbook re-run), failing early
with a clear message instead of a confusing package/service error.
