...
| trento_rabbitmq_host | The host where RabbitMQ is located. Prefer
  `trento_rabbitmq_port` for a non-default port; embedding the port
  directly here (eg. "myhost:5673") is still supported for
  backwards-compatibility. Used as input for
  calculating `trento_rabbitmq_effective_host` variable that is the
  default for `*_rabbitmq_host` variables in some dependent roles.
  This variable is ignored when `trento_rabbitmq_host_group` is
  non-empty string. | localhost
