...
`app_install_method` variable (either `docker` or `rpm`) to determine
which method to use.
...
....
{
  "postgres_web_password": "postgres",
  "postgres_wanda_password": "postgres",
...
`"app_enable_api_key": "false"` and skip `agent_web_api_key` altogether or
disable agent deployment for the first run, retrieve the api-key from
the UI and set the `agent_web_api_key` accordingly.
____
...
|postgres_wanda_password |Password of the postgres user used in wanda
project

|rabbitmq_password |Password of the rabbitmq user configured for the
trento projects

|rproxy_ssl_cert |String with the content of the .crt file to be used by
nginx for https
...
|===
|Name |Description
|agent_web_api_key |API key to connect to the trento_server

|agent_rabbitmq_password |Password of the rabbitmq user configured for the
trento projects
|===
...
|app_web_listen_port |Port where the Web service is exposed |<random value based on `app_server_name`>
...
|app_wanda_listen_port |Port where the Wanda service is exposed |<random value based on `app_server_name`>

|rproxy_wanda_location|Path to forward requests to Wanda in the proxy
|/wanda

|app_wanda_oas_server_url|Wanda OpenAPI server url
|https://<`trento_server_name`><`rproxy_wanda_location`>
...
|postgres_web_user |Name of the postgres user used by web application
|web

|postgres_web_db |Name of the postgres database of the web application
|webdb

|postgres_web_event_store |Name of the postgres event store database of
web application |event_store

|postgres_wanda_user |Name of the postgres user used by wanda project
|wanda

|postgres_wanda_db |Name of the postgres database of wanda application
|wanda

|app_web_postgres_host |Postgres host of web project container
|host.docker.internal

|app_wanda_postgres_host |Postgres host of wanda project container
|host.docker.internal
...
|app_secret_key_base |The secret of phoenix application |Generated by
playbook

|app_access_token_secret |The secret used for access tokens JWT signature
|Generated by playbook

|app_refresh_token_secret |The secret used for refresh tokens JWT signature
|Generated by playbook

|app_web_admin_username |Username of the admin user in web application
|admin

|app_web_admin_password |Password of the admin user of the web application

|app_enable_api_key |Enable/Disable API key usage. Mostly for testing
purposes |true

|app_enable_charts |Enable/Disable charts display based on Prometheus
metrics |true

|app_enable_alerting |Enable the alerting mechanism on web project |null

|app_alert_sender |Email address used as the "`from`" address in alerts
|null

|app_alert_recipient |Email address to receive alert notifications |null
...
|app_oidc_client_id |OIDC client id, required when `app_enable_oidc` is true |

|app_oidc_client_secret |OIDC client secret, required when `app_enable_oidc` is
true |

|app_oidc_server_base_url |OIDC identity provider base url, required when
`app_enable_oidc` is true |

|app_enable_oauth2 |Enable OAUTH2 integration, this disables the
username/password authentication method (self exclusive SSO type) |false

|app_oauth2_client_id |OAUTH2 client id, required when `app_enable_oauth2` is true
|

|app_oauth2_client_secret |OAUTH2 client secret, required when `app_enable_oauth2`
is true |

|app_oauth2_server_base_url |OAUTH2 identity provider base url, required
when `app_enable_oauth2` is true |

|app_oauth2_authorize_url |OAUTH2 authorize url, required when `app_enable_oauth2`
is true |

|app_oauth2_token_url |OAUTH2 token url, required when `app_enable_oauth2` is true
|

|app_oauth2_user_url |OAUTH2 user information url, required when
`app_enable_oauth2` is true |

|app_oauth2_scopes |OAUTH2 scopes, required when `app_enable_oauth2` is true
|"`profile email`"

|app_enable_saml |Enable SAML integration, this disables the
username/password authentication method (self exclusive SSO type) |false

|app_saml_idp_id |SAML IDP id, required when `app_enable_saml` is true |

|app_saml_idp_nameid_format |SAML IDP name id format, used to interpret the
attribute name. Whole urn string must be used
|urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified

|app_saml_sp_dir |SAML SP directory, where SP specific required files (such
as certificates and metadata file) are placed
|/etc/trento/trento-web/saml

|app_saml_sp_id |SAML SP id, required when `app_enable_saml` is true |
...
|app_saml_sp_org_displayname |SAML SP organization display name |"`SAML SP
build with Trento`"

|app_saml_sp_org_url |SAML SP organization url
|https://www.trento-project.io/

|app_saml_username_attr_name |SAML user profile "`username`" attribute field
name. This attribute must exist in the IDP user |username

|app_saml_email_attr_name |SAML user profile "`email`" attribute field name.
This attribute must exist in the IDP user |email

|app_saml_firstname_attr_name |SAML user profile "`first name`" attribute
field name. This attribute must exist in the IDP user |firstName

|app_saml_lastname_attr_name |SAML user profile "`last name`" attribute
field name. This attribute must exist in the IDP user |lastName

|app_saml_metadata_url |URL to retrieve the SAML metadata xml file. One of
`saml_metadata_url` or `saml_metadata_content` is required |

|app_saml_metadata_content |One line string containing the SAML metadata xml
file content (`saml_metadata_url` has precedence over this) |

|app_saml_sign_requests |Sign SAML requests in the SP side |true

|app_saml_sign_metadata |Sign SAML metadata documents in the SP side |true

|app_saml_signed_assertion |Require to receive SAML assertion signed from
the IDP. Set to false if the IDP doesn’t sign the assertion |true

|app_saml_signed_envelopes |Require to receive SAML envelopes signed from
the IDP. Set to false if the IDP doesn’t sign the envelopes |true

|rproxy_install |Install nginx |true

|rproxy_ssl_cert_as_base64 |Nginx SSL certificate provided as base64
string |false

|rproxy_ssl_key_as_base64 |Nginx SSL key provided as base64 string |false

|rproxy_override_nginx_default_conf |Override the default nginx conf for one
that will use the vhosts according to an opinionated directory structure
|true

|rproxy_vhost_filename |Nginx vhost filename. "`conf`" suffix is added to
the given name |trento

|rproxy_vhost_http_listen_port |Configure the http listen port for trento
(redirects to https by default) |80

|rproxy_vhost_https_listen_port |Configure the https listen port for
trento |443

|rproxy_web_upstream_name |Web nginx upstream name |web

|rproxy_wanda_upstream_name |Wanda nginx upstream name |wanda

|app_amqp_protocol |Change the amqp protocol type for Web and Wanda |amqp

|app_prometheus_url |Prometheus server url |http://localhost:9090

|prometheus_web_url |Base URL for Trento `web` where Prometheus would scrape for metrics |http://localhost

|app_install_method |Installation method for trento components, can be
...
|===
|Name |Description |Default
|agent_trento_server_url |Trento server url |http://localhost:4000

|agent_trento_repository |OBS repository from where trento agent is installed
|https://download.opensuse.org/repositories/devel:sap:trento:factory/SLE_15_SP3/

|agent_rabbitmq_username |Username of rabbitmq user, this will be created by
the rabbitmq role |trento

|agent_rabbitmq_host |The rabbitmq host, used by web and wanda containers. It
could include the service port |

|agent_amqp_protocol |Change the amqp protocol type |amqp
|===
